...
export function* saveFilter({
  ...
  try {
    ...
    yield eff.put(uiActions.setUiState('saveFilterDialogOpen', false));
    yield eff.put(uiActions.setUiState('issuesSourceId', filter.id));
    yield eff.put(uiActions.setUiState('filterStatusesIsFetched', false));
    ...
    if (errObj.body.errorMessages.length > 0) {
      ...
      yield eff.put(uiActions.setUiState('newJQLFilterErrors', errObj.body.errorMessages));
    } else {
      yield eff.fork(notify, {
        title: 'Failed to save filter',
